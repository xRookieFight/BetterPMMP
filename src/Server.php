...
class Server{
	...
	public const DEFAULT_SERVER_NAME = "A " . VersionInfo::NAME . " Server";
	...
	public function reload(){
		$this->logger->info("Dünyalar kaydediliyor..");

		foreach($this->worldManager->getWorlds() as $level){
			$level->save();
		}
		...
		foreach (PermissionManager::getInstance()->getPermissions() as $permission) {
			if (!str_starts_with($permission->getName(), "pocketmine")){
				PermissionManager::getInstance()->removePermission($permission);
			}
		}
		$this->commandMap->clearCommands();

		$this->logger->info("Sunucu ayarları yenileniyor...");
